...
class GradeAndGenerateTool(object):
    ...
    def __init__(self):
        ...
        """
        self.grade_messages = [SystemMessage(content=self.system_prompt),
                         HumanMessage(content="你好"),
                         AIMessage(content="你好！请问有什么能够帮您？")]


        self.hallucinations_prompt = "您是一名评估LLM生成是否基于一组检索到的事实的评分员。如果是基于检索到的事实回答则返回no，否则返回yes"
        self.hallucinations_messages = [SystemMessage(content=self.hallucinations_prompt),
                         HumanMessage(content="你好"),
                         AIMessage(content="你好！请问有什么能够帮您？")]



        self.answer_question_prompt = """
        你是一名评分员，评估答案是否解决了问题，如果解决了则返回yes，否则返回no
        """

        self.answer_question_messages = [SystemMessage(content=self.answer_question_prompt),
                         HumanMessage(content="你好"),
                         AIMessage(content="你好！请问有什么能够帮您？")]


        self.rewrite_promtp = "您是一个将输入问题转换为优化的更好版本的问题重写器\n用于矢量库检索。查看输入并尝试推理潜在的语义意图/含义。"
        self.rewrite_promtp_messages = [SystemMessage(content=self.rewrite_promtp),
                         HumanMessage(content="你好"),
                         AIMessage(content="你好！请问有什么能够帮您？")]
    ...
    def grade(self, question, text):
        self.grade_messages.append(HumanMessage(content=f"问题：{question}\n文档：{text}"))
        result = self.struct_llm_grader.invoke(self.grade_messages)
        self.grade_messages.append(AIMessage(content=result.binary_score))
    ...
    def generate(self, question, text):
        ...
        grade_human_prompt_end = human_prompt.format_messages(question=question, context=text)
        result = self.llm.invoke(grade_human_prompt_end)
        return result.content

    # 判断是否有幻觉
    def hallucinations(self, documents, answer):
        self.hallucinations_messages.append(HumanMessage(content=f"：回答:{answer}\n文档：{documents}"))
        result = self.struct_llm_hallucinations.invoke(self.grade_messages)
        self.grade_messages.append(AIMessage(content=result.binary_score))
        return result.binary_score

    # 判断答案是否和问题相关
    def answer_question(self, question, answer):
        self.answer_question_messages.append(HumanMessage(content=f"问题：{question}\n回答：{answer}"))
        result = self.struct_llm_answer.invoke(self.answer_question_messages)
        self.answer_question_messages.append(AIMessage(content=result.binary_score))
        return result.binary_score


    # 复写问题
    def rewrite_question(self, question):
        self.rewrite_promtp_messages.append(HumanMessage(content=f"问题：{question}"))
        result = self.llm.invoke(self.rewrite_promtp_messages)
        self.rewrite_promtp_messages.append(AIMessage(content=result.content))
        return result.content
    ...
    def search_vector(self, question):
        result = self.milvus_client.search(collection_name="rag", data=[self.embed_dim(question)], output_fields=["text"])
        return result
